...
web.config.session_parameters['timeout'] = config.getConfig().timeout
...
#创建session对象，并进行初始化
session = web.session.Session(app, web.session.DiskStore('sessions/test'), initializer = {'isLogin': 0 , 'isSuper':'NOT'})
